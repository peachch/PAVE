...
def semantic_counter(claim, label, api, model, temperature, max_tokens, extra_body):
    ...
    prompt = (
        ...
        f"judge the following claim as {target}. The passage should directly address the claim and "
        "should not mention that it is fabricated or counterfactual. Output only the evidence passage.\n\n"
        f"Claim: {claim}\n"
        f"Target verdict: {target}\n"
        "Evidence:"
    )
    return api.ask(prompt, model, temperature, max_tokens, extra_body).strip()
...
def entity_counter(
    ...
    extraction_prompt = (
        "Extract the named entities or concrete values in the evidence that are directly relevant "
        "to judging the claim. "
    ...
    for entity in entities:
        replacement_prompt = (
            "Replace the following entity with a plausible entity of the same semantic type, but "
            "with a clearly different identity or value. Output only the replacement.\n\n"
            f"Entity: {entity}\nReplacement:"
        )
        replacement = api.ask(
            replacement_prompt, model, temperature, max_tokens, extra_body
        ).strip().strip('"“”')
        if not replacement or replacement.lower() == entity.lower():
            return ""
